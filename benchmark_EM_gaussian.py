...
try:
    from single_run_EM_gaussian import run_single_trial
except ImportError:
    raise ImportError("Can't find single_run_em_gaussian.py")
...
if __name__ == "__main__":
    ...
    for config in configurations:
        for _ in range(N_RUNS):
            tasks.append(
                (tid, config['outlier_rate'], config['missing_rate'], tid)
            )
            tid += 1

    print(f" Benchmark Gaussian EM | {len(tasks)} runs")
    ...
    filename = f"RESULTS_EM_GAUSSIAN_{timestamp}.csv"
    ...
    if not df_clean.empty:
        print(
            df_clean
            .groupby(['outlier_rate', 'missing_rate'])[
                ['fisher_dist', 'frobenius_rel', 'err_mu']
            ]
            .mean()
        )
